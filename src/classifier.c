...
{
    float* v = xcalloc(n, sizeof(float));
    int i;
    for(i = 0; i < n; ++i){
...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dontuse_opencv, int dont_show, int mjpeg_port, int calc_topk, int show_imgs, char* chart_path)
{
    ...
    printf("%s\n", base);
    printf("%d\n", ngpus);
    network* nets = xcalloc(ngpus, sizeof(network));
...
void validate_classifier_10(char *datacfg, char *filename, char *weightfile)
{
    ...
    float avg_acc = 0;
    float avg_topk = 0;
    int* indexes = xcalloc(topk, sizeof(int));
    ...
    for(i = 0; i < m; ++i){
        ...
        float* pred = xcalloc(classes, sizeof(float));
        for(j = 0; j < 10; ++j){
...
void try_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename, int layer_num)
{
    ...
    clock_t time;
    int* indexes = xcalloc(top, sizeof(int));
    char buff[256] = { 0 };
    char *input = buff;
    while(1){
        if(filename){
